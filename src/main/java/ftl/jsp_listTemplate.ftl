  ...
  <div class="layui-form-item">
		...
		<tbody>
			<c:if test="${r"${not empty list}"}">
				<c:forEach items="${r"${list}"}" var="a">
					...
					<td><fmt:formatDate value="${r"${a.lastUpdateTime }"}"  pattern="yyyy-MM-dd HH:mm:ss"/> </td>
					<td>${r"${a.lastUpdateUser }"}</td>
					<td><fmt:formatDate value="${r"${a.lastUpdateTime }"}"  pattern="yyyy-MM-dd HH:mm:ss"/> </td>
					<td>${r"${a.createUser }"}</td> 
						<td>
							<div class="layui-btn-group">
							<button class="layui-btn layui-btn-small" onclick="edit('${r"${a.id}"}');">编辑</button>
							
							<button class="do-action layui-btn layui-btn-small layui-btn-danger" 
											data-type="ajaxDelete" data-url="<%=basePath%>${module}/${componentLower}/delete/${r"${a.id}"}">删除</button>
							</div>
						</td>
					</tr>
				</c:forEach>
	...
	<div id="page"></div>
		...
		<div class="layui-tab-item">
			<iframe src="<%=basePath%>${module}/${componentLower}/badd" id= "add_iframe" height=600 scrolling="no" frameborder="0"></iframe>
		</div>
	</div>
</div>	
...
loadJsonDataToForm('${r"${el:toJsonString(query)}"}','query')
...
laypage({
    ...
    ,groups: 5 //连续显示分页数
    ,last:${r"${page.totalPage}"}
    ,curr:${r"${page.currentPage}"}
    ,jump:function(obj,first){
    	if(first != true){
    		debugger;
    		${r"$('#currentPage')"}.val(obj.curr);
	    	${r"$('#query')"}.submit();
    	}
    }
});
 ...
 function del(id){
 	var url = "<%=basePath%>${module}/${componentLower}/delete/"+id;
  ...
  function edit(id){
     ...
     var iframe = '<iframe id ="iframe_eidt_'+id+'" src="<%=basePath%>${module}/${componentLower}/bedit/'+id+'" style="height:' + mainHeight + 'px;width:100%"></iframe>';
