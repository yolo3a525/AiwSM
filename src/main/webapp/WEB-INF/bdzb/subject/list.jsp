...
<div class="layui-tab layui-tab-brief sub-page-tab" lay-filter="sub_tab">
			...
			<c:if test="${not empty list}">
				<c:forEach items="${list}" var="a">
					...
					<td>${a.id }</td>
						...
						<td>
						 <c:choose> 
							  <c:when test="${not empty a.imgFlag && a.imgFlag == 1}">
							  	<img alt="" src="<%=basePath%>upload/subject/${a.id}.jpg?r=<%=r%>" width="300">
							  </c:when>
							  	<c:otherwise>
							    没有图片
							  </c:otherwise>
					...
					<td>${a.createUser }</td> 
						<td>
							...
							<button class="layui-btn layui-btn-small" onclick="preview('${a.id}');">预览</button>
							...
							<button class="do-action layui-btn layui-btn-small layui-btn-danger" 
											data-type="ajaxDelete" data-url="<%=basePath%>bdzb/subject/delete/${a.id}">删除</button>
							</div>
						</td>
					</tr>
				</c:forEach>
	...
	<div id="page"></div>
		...
		<div class="layui-tab-item">
			<iframe src="<%=basePath%>bdzb/subject/badd" id= "add_iframe" height=800 scrolling="no" frameborder="0"></iframe>
		</div>
	</div>
</div>	
...
laypage.render({
 ...
 function del(id){
 	var url = "<%=basePath%>bdzb/subject/delete/"+id;
  ...
  function edit(id){
     ...
     var iframe = '<iframe id ="iframe_eidt_'+id+'" src="<%=basePath%>bdzb/subject/bedit/'+id+'" style="height:' + mainHeight + 'px;width:100%"></iframe>';
  ...
  function preview(id){
		parent.layer.open({
	        type: 2
	        ,id: 'preview'
	        ,content: "<%=basePath%>bdzb/subject/preview/"+id
	        ,area: ['600px','800px']
	    });
	}
